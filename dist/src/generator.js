...
function generate(ast, options) {
    ...
    return ([
        ...
        lodash_1.uniq(declareImports(ast, options, ast.standaloneName).split('\n')).join('\n'),
...
function declareImports(ast, options, rootASTName, processed) {
    ...
    switch (ast.type) {
        case 'CUSTOM_MODULE':
            ...
            break;
        case 'ARRAY':
            imports = declareImports(ast.params, options, rootASTName, processed);
            break;
        case 'INTERFACE':
            imports = [
                getSuperTypesAndParams(ast)
                    .map(function (ast) { return declareImports(ast, options, rootASTName, processed); })
                    .filter(Boolean)
                    .join('\n')
            ]
                .filter(Boolean)
        ...
        case 'UNION':
            imports = ast.params
                .map(function (_) { return declareImports(_, options, rootASTName, processed); })
                .filter(Boolean)
                .join('\n');
